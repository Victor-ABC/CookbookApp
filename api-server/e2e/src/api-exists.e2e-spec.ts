...
describe('exists', () => {
  ...
  it('should succeed given proper credentials: does not exist', async () => {
    ...
    const response = await userSession.post('users/exists', { name: userSession.signInData().name + 'error' });
  ...
  it('should succeed given proper credentials: does exist', async () => {
    await userSession.registerUser();
    await userSession.post('/users/sign-in', userSession.signInData());
    const response = await userSession.post('users/exists', { name: userSession.signInData().name });
